...
export default function PriceBox({ disp, orig, mod, modId, addUpdate, refreshLines, removeMod, sizing }){
  ...
  const changeModVal = (e) => {
    ...
    if(t.toUpperCase() == 'I' || t == 'Incl.'){
      ...
      new_value = (parseInt(e.target.value.replace(/[^0-9]/g,'')) / 100).toFixed(2);
    }
    if(new_value == 'NaN'){
      new_value = ''
    }
  ...
  const handleAddUpdate = async () => {
    const add = await addUpdate('price', modVal);
    add && reLine();
  }
  ...
  return (
      ...
      <div className={"cursor-default relative w-[10%] text-right " + (mod ? 'bg-yellow-500' : 'hover:bg-yellow-500')} style={{padding: sizing.defaultLine.p}} onClick={()=>startMod()} onContextMenu={(e)=>{mod && dispContext(e)}}>
        <div>
          {modify ? <input
                      className="decoration-none appearance-none text-right w-full outline-none bg-transparent"
                      value={modVal}
